...
# creating new object from data frame
drought_exp_clean <- drought_exp |> 
  # clean column names by lowercasing and replacing spaces with underscores
  clean_names() |> 
  ...
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # reordering treatment name levels
...
```{r}
#| label: sla and plant mass visualization
...
ggplot(
  ...
  labs(#x = expression("Specific leaf area(cm"^2~"/g)") # does the same thing as below
       x = "Specific leaf area (cm\U00B2 / g)", # superscript for cm^2
       y = "Total mass (g)",
...
```{r}
#| label: mass and water treatment visualization
...
par(mfrow = c(2, 2))
# plotting diagnostics of model 4
plot(model4)

```

- residuals are normally distributed (QQ plots)
- residuals are homoscedastic (Residuals vs Fitted/Scale-Location)
- no outliers that could be influencing model (Constant Leverage)
...
The best model includes water treatment and species as indicators.

**How much variation in the response (total mass, in grams) does this model explain?**

72.6% of the variation in plant mass is explained with this model.
...
- `species_nameEschscolzia californica`: difference between Eschscholzia californica mass in the well watered treatment and Lotus scoparius mass in the well watered treatment.
...
# renaming column to fit original data frame
...
ggplot(
  ...
  # y-axis: total mass
  # coloring by water treatment
  mapping = aes(x = water_treatment,
                y = total_g,
                color = water_treatment)) +
  # first layer: jitter to represent individual plants for each treatment
  geom_jitter(height = 0,
              width = 0.1,
              alpha = 0.4,
              shape = 21) +
  # second layer: pointrange to represent model predictions
  geom_pointrange(data = preds,
                  mapping = aes(x = water_treatment,
                                y = predicted,
                                ymin = conf.low,
                                ymax = conf.high)) +
  # creating plots for each species
